...
#define OV7670_INIT_INPUTS \
              const int inputPins[]={PC0,PA1,PA2,PC3,PC4,PC5,PC6,PC7,PD0,PD3, PD4}; \
              for(int i=0;i<8;i++) { \
                pinMode(inputPins[i],INPUT); \
              }

// vsync - PD4
#define OV7670_VSYNC ((*GPIOD).INDR & 0x0010)

// pixel clock - PD0
#define OV7670_PIXEL_CLOCK ((*GPIOD).INDR & 0x0001)

// href - PD3
#define OV7670_HREF ((*GPIOD).INDR & 0x0008)

// pixel byte - PC0- PC7
// #define OV7670_READ_PIXEL_BYTE(b) b = ((uint8_t*)(&(*GPIOD).INDR))[1]
#define OV7670_READ_PIXEL_BYTE(b) \
            b=((*GPIOC).INDR & 0xF9 )|((*GPIOA).INDR & 0x06 );\
            // b=((((uint8_t*)(&(*GPIOC).INDR))[1]) & 0xF9 )|((((uint8_t*)(&(*GPIOA).INDR))[1]) & 0x06 );
                    // ((*GPIOC).INDR & 0x00F9 )|((*GPIOA).INDR & 0x0006 );
